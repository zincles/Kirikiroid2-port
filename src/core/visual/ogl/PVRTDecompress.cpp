/* Vendored for this port (Kirikiroid2, Linux/SDL2, no cocos2d-x):
   source cocos2d-x 3.6 cocos/base/pvr.cpp, MIT licensed,
   https://raw.githubusercontent.com/cocos2d/cocos2d-x/cocos2d-x-3.6/cocos/base/pvr.cpp
   which in turn carries Imagination Technologies' PVRTDecompress implementation
   (see the copyright header below). Kirikiroid2's Android build got this
   implementation from its cocos2d-x vendor tree; that tree is not part of this
   port, so the file is vendored here. Only the include of the declaration
   header was adapted; the code is untouched. */
/******************************************************************************
